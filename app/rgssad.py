...
def shorten(number):
    #print("incoming "+hex(number))
    number = number & 0xFFFFFFFF
    #print("shortened "+hex(number))
    return number
...
def read():
    ...
    GameArchive = open('./Game.rgssad', "rb")
    GameArchive.seek(0,2)
    ArchiveSize = GameArchive.tell()
    print("Archive size "+hex(ArchiveSize))
    GameArchive.seek(8,0)
    
    while True:
        print(">> It's a new cycle<<")
        length = DecryptInteger(GameArchive.read(4))
        filename = DecryptName(GameArchive.read(length))
        size = DecryptInteger(GameArchive.read(4))
        print(GameArchive.tell())
        offset = GameArchive.tell()
        File = EncryptedData(filename, offset, size, key)
        ArchivedData.append(File)
        GameArchive.seek(size, 1)
        print("file name "+filename)
        print("file size "+str(size))
        print("offset "+hex(offset))
        print("new key "+hex(key))
        print("we're at"+hex(GameArchive.tell()))
        if GameArchive.tell()==ArchiveSize:
            break
    print("holy shit, we made it.")
    DecryptFiles(GameArchive, ArchivedData)
